...
codigo = "!init $MyProgam ; var $id1 ; $id2 ; $id3 ; : integer ; { if true == true then { $id1 = 1 $id2 = 2 $id3 = 3 $id4 = 20 $id1 = 55 } else { $id1 = 5 $id2 = 9 $id3 = 10 $id1 = 55 } endif } !end"
    ...
    for k,v in find_identifiers_and_literals(codigosplit[0]).items():
        if len(v) > 0:
            ...
            if k == 'identifiers':
                ...
                if auxpaso == 'DECLARE':
                    if v not in symbol_table and v not in valid:
                        valid.append(v)
                        symbol_table.append({"var": v})
                    else:
                        print("-------------SEMANTIC ERROR--------------")
                        print(f'El identificador {codigosplit2[0]} ya está declarado')
                        quit()
                elif auxpaso == 'ASSING':
                    ele = any(codigosplit2[0] in dic['var'] for dic in symbol_table)

                    if ele:
                        print('ID in list')
                    else:
                        print("-------------SEMANTIC ERROR--------------")
                        print(f'El identificador {codigosplit2[0]} no está declarado')
                        quit()
